...
const PDFDiagnosticoGeral = () => {
  ...
  const [
    mediaformularioPerguntaSemaparada,
    setMediaformularioPerguntaSemaparada,
  ] = useState([]);
  const [todasRespostas, setTodasRespostas] = useState([]);
  const [mediaFormularioGeralempresa, setmediaFormularioGeralempresa] =
    useState([]);
    ...
    const calcularMediaFormularioEmpresa = async () => {
      for (let j = 0; j < empresa.length; j++) {
        for (let i = 0; i < formulario.length; i++) {
          ...
          const media =
            soma / (todasRespostas.length * todasRespostas[0].respostas.length);
